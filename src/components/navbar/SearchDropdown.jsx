...
export default function SearchDropdown({ open, active, anchorRef, value, onApply, onClose, onTabChange }) {
  ...
  const anchoredStyle = useMemo(() => {
    ...
    if (active === "where") return { position: "fixed", top, left: Math.max(pad, rect.left), zIndex: 110 };
    if (active === "who") return { position: "fixed", top, right: Math.max(pad, vpW - rect.right), left: "auto", zIndex: 110 };
  ...
  return (
    <>
      <div className={`fixed inset-0 z-[100] bg-black/20 transition-opacity ${show ? "opacity-100" : "opacity-0"}`} onClick={onClose} aria-hidden="true" />
      ...
      <div style={anchoredStyle} className="hidden lg:block">
        <div className={["relative w-[min(920px,calc(100vw-64px))] rounded-[32px] bg-white","shadow-[0_6px_20px_rgba(0,0,0,0.2)] ring-1 ring-black/5","transition-all duration-300", show ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2","px-8 pb-10 pt-6"].join(" ")} role="dialog" aria-modal="true">
          ...
          {(active === "checkin" || active === "checkout" || active === "when") && dateTab === "dates" && (
            <DatesPanel leftMonth={leftMonth} rightMonth={rightMonth} startDate={startDate} endDate={endDate} onDayClick={handleDayClick} flex={flex} setFlex={setFlex} offset={offset} setOffset={setOffset} />
          )}
          ...
          <div className="p-6">
            ...
            {(active === "checkin" || active === "checkout" || active === "when") && (
              <>
                <div className="mb-4 flex w-full items-center justify-center">
                  <div className="rounded-full bg-[#EBEBEB] p-1 shadow-sm">
                    {["dates", "months", "flexible"].map((t) => {
                      const isActive = dateTab === t;
                      return (
                        <button key={t} role="tab" aria-selected={isActive} onClick={() => { setDateTab(t); onTabChange?.(t); }} className={["px-6 py-2.5 text-[14px] font-medium rounded-full transition outline-none focus-visible:ring-2 focus-visible:ring-black/10", isActive ? "bg-white shadow-sm text-black font-semibold" : "text-[#222222] hover:text-[#717171]"].join(" ")}>
                          {t[0].toUpperCase() + t.slice(1)}
                        </button>
                      );
                    })}
                ...
                {dateTab === "dates" && <DatesPanel leftMonth={leftMonth} rightMonth={null} startDate={startDate} endDate={endDate} onDayClick={handleDayClick} flex={flex} setFlex={setFlex} offset={offset} setOffset={setOffset} />}
                {dateTab === "months" && <MonthsPanel startDate={startDate} endDate={endDate} setStartDate={setStartDate} setEndDate={setEndDate} />}
...
function WherePanel({ query, setQuery, location, setLocation }) {
  ...
  return (
    <div className="w-full max-w-[520px]">
      <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search destinations" className="w-full rounded-2xl border border-[#DDDDDD] bg-white px-4 py-3 text-[15px] outline-none focus:ring-2 focus:ring-black/10" />
      <div className="mt-4 text-[14px] font-semibold text-[#222222]">Suggested destinations</div>
      <div className="mt-2 max-h-[320px] overflow-auto pr-2">
        {filtered.map((s) => (
          <button key={s.id} type="button" onClick={() => setLocation({ id: s.id, name: s.name })} className={["w-full rounded-2xl px-3 py-2 transition text-left", "hover:bg-[#F7F7F7]", location?.id === s.id ? "ring-2 ring-black/10" : ""].join(" ")}>
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#F7F7F7]">{s.icon}</div>
              <div>
...
function DatesPanel({ leftMonth, rightMonth, startDate, endDate, onDayClick, flex, setFlex, setOffset }) {
  ...
  const Month = (monthDate) => {
    ...
    return (
      <div className="w-full">
        <div className="mb-4 text-center text-[18px] font-semibold text-[#222222]">{label}</div>
        <div className="grid grid-cols-7 gap-y-2 text-center text-[12px] font-semibold uppercase text-[#717171]">{DAYS.map((d) => <div key={d} className="py-1">{d}</div>)}</div>
        <div className="mt-1 grid grid-cols-7 gap-y-1">
          {cells.map((d, i) => {
            ...
            return (
              <button key={i} type="button" disabled={disabled} onClick={() => onDayClick(d)} className={["mx-auto my-1 flex h-12 w-12 items-center justify-center rounded-full text-[14px] transition", disabled ? "text-[#DDDDDD] cursor-not-allowed" : "text-[#222222] hover:bg-[#F7F7F7]", start || end ? "bg-black text-white hover:bg-black" : "", !start && !end && between ? "bg-[#F7F7F7] text-black" : ""].join(" ")}>
                {d ? d.getDate() : ""}
              </button>
            );
          })}
  ...
  return (
    <div className="w-[min(920px,calc(100vw-64px))]">
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="grow" />
        <button type="button" onClick={() => setOffset((v) => v + 1)} className="inline-flex h-9 w-9 items-center justify-center rounded-full hover:bg-[#F7F7F7]" aria-label="Next month"><ChevronRight className="h-5 w-5" /></button>
      </div>

      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">{Month(leftMonth)}{rightMonth && <div className="hidden lg:block">{Month(rightMonth)}</div>}</div>

      <div className="mt-6 flex flex-wrap items-center justify-center gap-2">{quick.map((d) => {
        const active = flex === d;
        return <button key={d} type="button" onClick={() => setFlex(d)} className={["rounded-full border px-4 py-2 text-[14px] transition", active ? "bg-black text-white border-black" : "bg-white text-black border-[#B0B0B0] hover:border-[#717171]"].join(" ")}>{d === 0 ? "Exact dates" : `± ${d} ${d === 1 ? "day" : "days"}`}</button>;
      })}</div>
    </div>
  );
}
...
function MonthsPanel({ startDate, endDate, setStartDate, setEndDate }) {
  ...
  const toAngleFromCenter = (cx, cy, x, y) => {
  ...
  const angleForMonths = (m) => (m / 12) * 300 + 30;
  ...
  const onPointer = (e) => {
    ...
    const angle = toAngleFromCenter(cx, cy, e.clientX, e.clientY);
    let raw = ((angle - 30 + 360) % 360) / 300;
    raw = clamp(raw, 0, 1);
  ...
  const angle = angleForMonths(months);
  ...
  return (
    <div className="w-[min(920px,calc(100vw-64px))] min-h-[600px]">
      ...
      <div ref={dialRef} className="mx-auto my-6 h-80 w-80 rounded-full relative flex items-center justify-center" role="slider" aria-label="Select number of months" aria-valuemin={1} aria-valuemax={12} aria-valuenow={months} tabIndex={0} onKeyDown={onKeyDown} onPointerDown={onPointerDown} style={{ background: `conic-gradient(#FF385C ${angle}deg, rgba(0,0,0,0.06) ${angle}deg)`, boxShadow: "inset 0 10px 25px rgba(0,0,0,.08), inset 0 -6px 12px rgba(0,0,0,.06), 0 2px 12px rgba(0,0,0,.06)" }}>
        ...
        <div className="h-40 w-40 rounded-full bg-white shadow-md flex items-center justify-center text-center">
          <div>
            ...
            <div className="text-sm text-[#717171]">{months === 1 ? "month" : "months"}</div>
          </div>
        </div>
      ...
      <div className="mt-8 text-center text-[15px] text-[#222222]"><span className="underline">{fmt(startDate || baseStart)}</span><span className="mx-2 text-[#717171]">to</span><span className="underline">{fmt(endDate || new Date(baseStart.getFullYear(), baseStart.getMonth() + months, 1))}</span></div>
    </div>
  );
}
  ...
  return (
    <div className="w-[min(920px,calc(100vw-64px))] min-h-[600px]">
      ...
      <div className="mb-8 flex items-center justify-center gap-3">{[
        ...
        return <button key={opt.key} type="button" onClick={() => setLength(opt.key)} className={["rounded-full border px-7 py-3 text-[16px] transition", active ? "bg-black text-white border-black" : "bg-white text-black border-[#B0B0B0] hover:border-[#222222] hover:shadow"].join(" ")}>{opt.label}</button>;
      })}</div>
...
function GuestsPanel({ guests, setGuests }) {
  ...
  const update = (key, delta) => setGuests((g) => ({ ...g, [key]: Math.max(0, (g[key] || 0) + delta) }));
        ...
        {rows.map((r) => {
          ...
          return (
            <div key={r.key} className="flex items-center justify-between py-6">
              ...
              <div className="flex items-center gap-3">
                <button type="button" onClick={() => update(r.key, -1)} disabled={count === 0} className={["inline-flex h-8 w-8 items-center justify-center rounded-full border bg-white", count === 0 ? "border-[#EBEBEB] text-[#B0B0B0]" : "border-[#B0B0B0] text-[#222222] hover:border-[#717171]"].join(" ")}>
                  <Minus className="h-4 w-4" />
                </button>
                <div className="w-8 text-center text-[16px]">{count}</div>
                <button type="button" onClick={() => update(r.key, +1)} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[#B0B0B0] bg-white text-[#222222] hover:border-[#717171]">
